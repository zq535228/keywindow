...
namespace 快捷键窗体切换 {
    public partial class MainForm :Form {
        ...
        private Dictionary<IntPtr, Form> borderForms = new Dictionary<IntPtr, Form>();
        ...
        private void InitializeUI() {
            ...
            // 添加顶置窗口选项
            ToolStripMenuItem topmostItem = new ToolStripMenuItem("一键顶窗设置");
            topmostItem.Click += TopmostItem_Click;
        ...
        private void SwitchToWindow(string processName) {
            try {
                // 检查是否是特殊命令
                if(processName.Equals("TOGGLE_TOPMOST" , StringComparison.OrdinalIgnoreCase)) {
                    ToggleWindowTopMost();
                    return;
                }
        ...
        private void MainForm_FormClosing(object sender , FormClosingEventArgs e) {
            ...
            // 清理所有边框窗口
            foreach(var borderForm in borderForms.Values) {
                borderForm.Close();
                borderForm.Dispose();
            }
            borderForms.Clear();
        ...
        private void ToggleWindowTopMost() {
            try {
                ...
                // 设置或清除窗口边框颜色
                if(!isTopMost) {
                    // 设置为淡红色边框 (RGB: 255, 128, 128)
                    int color = (255 << 16) | (128 << 8) | 128;
                ...
                MessageBox.Show($"切换窗口置顶状态失败: {ex.Message}");
            }
        }
        ...
        private void TopmostItem_Click(object sender , EventArgs e) {
            this.TopMost = false;  // 暂时取消主窗口置顶
            using(var configForm = new HotkeyConfigForm()) {
                // 预设置应用名称为TOGGLE_TOPMOST
                configForm.SetProcessName("TOGGLE_TOPMOST");

                // 查找是否已存在顶置窗口的配置
                var existingConfig = hotkeyConfigs.Values.FirstOrDefault(c => c.ProcessName == "TOGGLE_TOPMOST");
                if(existingConfig != null) {
                    configForm.SetConfig(existingConfig);
                }
                ...
                if(configForm.ShowDialog() == DialogResult.OK) {
                    ...
                    if(config != null) {
                        // 如果已存在配置，先删除旧的
                        if(existingConfig != null) {
                            var oldHotkeyId = hotkeyConfigs.FirstOrDefault(x => x.Value == existingConfig).Key;
                            UnregisterHotKey(this.Handle , oldHotkeyId);
                            hotkeyConfigs.Remove(oldHotkeyId);

                            // 从ListView中移除旧项
                            foreach(ListViewItem item in listView.Items) {
                                if(item.SubItems[1].Text == "TOGGLE_TOPMOST") {
                                    item.Remove();
                                    break;
                                }
                            }
